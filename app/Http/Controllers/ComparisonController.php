...
class ComparisonController extends Controller
{
    ...
    public function viewPage(Request $Request){
        ...
        $ID1 = $CollectionID->first();
        $ID2 = $CollectionID->diff($ID1);

        $usedcar1 = UsedCar::find($collection[0]->id);
        $usedcar2 = UsedCar::find($collection[1]->id);

        // Data1
        $Inspection1 = Inspection::select('result_file')->where('used_car_id',$ID1)->latest()->first();
        ...
        // Data1
        $Inspection2 = Inspection::select('result_file')->where('used_car_id',$ID2)->latest()->first();
